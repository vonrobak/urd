...
| `effective policy` / `declared intent` | `declared intent` = what the user's config says (`ResolvedSubvolume.local_retention` / `send_interval`). `effective policy` = `EffectivePolicy { local_retention, send_interval, clear_all }`, the tier-adapted operational policy the planner, executor, **and** awareness all act on. The planner and awareness MUST derive the same effective policy from the **same** armed tier (the single pre-plan gather, `backup.rs`) or a correctly-adapting subvolume shows false AT RISK. Since UPI 082-a the full policy has **one** derivation site — the planner — which records the executor-facing slice on the plan as `PlannedLifecycle { is_transient, clear_all, shed_away_drives }` (the executor consumes the plan, never re-derives); awareness consumes the extracted `effective_send_interval` projection, which carries no away-pin parameter by construction. |
| `RunArming` | The run-scoped artifact resolved **once** at the pre-plan gather (UPI 082-a): `{ armed_tier_map, pools (writeback rows), away_shed }` — the per-subvolume armed tiers, the per-pool rows `advance_and_writeback` persists, and the away-sheddable pin view. The planner (`plan(&RunArming)`), the watchdog arming, the abort-reclaim, and the writeback all read this one object; act-time consumers (the executor's in-run shed, the reclaim's S3 re-filter) **re-confirm presence** against live state before acting, they never re-derive the policy. Built pre-lock with the gather; the emergency re-plan reuses it (AB1). Extends the pre-082 `ResolvedArmedTiers`. |
...
| `mid-op watchdog` | ADR-113 **Layer 2** (UPI 033; floor-only since UPI 067): an in-process sibling thread (modelled on `progress_display_loop`) that polls source-pool free **level** *during* sends, the window the pre-flight planner and post-delete executor cannot see. Pure decision core in `guard.rs` (`evaluate` → `WatchdogAction`); the thread, cancel plumbing, and abort-reclaim are the wiring in `commands/backup.rs`. Armed only on Tight/Critical source pools with a send-enabled subvolume; not TTY-gated (autonomous runs need it most). |
| `floor` | The watchdog's **sole** trigger (floor-only since UPI 067): the absolute level `min_free + cleanup_budget`, read against *current* free — free below it aborts the in-flight send, else continue. The watchdog degrades it to bare `min_free` for a pool that *started* below the floor (UPI 054-a, a plan→start TOCTOU residual the planner allowed). *Historical:* a second differential `cliff` trigger (write-rate, windowed-averaged per UPI 065-a) was the documented **primary** until UPI 067 deleted it — its entire production record was phantom and destructive (2/2 firings, both `cliff_exceeded`), while the floor never fired. The floor's late-but-safe error direction (fire late → caught by the catastrophic floor / ENOSPC, never sever a chain on a transient) is exactly why it is the trigger that stays. |
| `reserve file` / `.urd-emergency-reserve` | *(Historical — retired UPI 067.)* A pre-allocated 1 GiB regular file at each armed pool's snapshot root, the watchdog's **fast bridge** (UPI 033): deleting it freed real extents at the next transaction commit (faster than btrfs's async subvolume-delete cleaner), buying runway while the definitive snapshot-shed committed. Allocated with `fallocate` (real extents, exempt from transparent compression) — **never** zero-byte-written, which would free ~nothing on a compress mount (C2). Deleted with the cliff in UPI 067 (the bridge only ever mattered for the rate trigger); `reserve.rs` is gone, and a one-release `sweep_orphaned_reserves` reclaims the `fallocate`'d remnants left on disk. |
| `cleanup_budget` | The **derived** working room the watchdog floor keeps free above `min_free` on the source pool (`floor = min_free + cleanup_budget`): fixed **1.5 % of pool capacity** (`CLEANUP_BUDGET_CAPACITY_FRACTION`), resolved where the capacity is in scope (`guard::source_floor_bytes`). *Historical:* was a per-`snapshot_root` config field (`Option<ByteSize>`, added UPI 033), retired in UPI 068 — never set in practice, so only the derived default was ever exercised; the parsers tolerate the residual key and `urd migrate` warns-and-drops it (ADR-111 amendment 2026-07-02). |
| `abort-reclaim` | The definitive source-pool reclaim after a watchdog abort (UPI 033 Step 5b, `executor::emergency_reclaim_pool`). Cancelling a `btrfs send` frees **no** source space on its own (the pressure is the retained snapshot's CoW growth + ambient writes; the only wired cleanup deletes the *destination* partial — wrong pool). So once the send exits, Urd reclaims the **triggering pool's** local snapshots, reusing the 031-b fail-closed ordering, **but only for subvolumes with a confirmed offsite copy** (a pin): a never-sent subvolume is skipped, never delete the only stored copy. **Two-tier and graduated since UPI 058 (ADR-116):** Tier 1 `shed`s only the away-*only* pins, syncs, and re-measures against the `catastrophic floor` via an injected free-probe; if that cleared the floor the **connected chains survive**. Tier 2 (escalation — a single below-floor reading, an unavailable probe, or nothing away to shed; bias-to-escalate) is the pre-058 **blanket** clear of every pin, the host-survival guarantee. Host survival > chain continuity: a dropped pin makes the next send full (the documented acceptable cost). An ADR-106-scoped exception **authorized by** ADR-113's catastrophic-floor doctrine; the live subvolume is never touched and falls back to its prior offsite copy. **Pool-scoped since UPI 065-b (ADR-113 2026-06-17 amendment):** the response is partitioned by the in-flight send's source filesystem. *Same-filesystem* (the live send reads the tripping pool) aborts the send and runs this reclaim post-abort on the **main thread** at teardown, exactly as above. *Cross-filesystem* (the live send reads a different, independent pool) leaves that send untouched and reclaims the tripping pool's own snapshots **concurrently on the watchdog thread** — safe by construction (disjoint devices; the `WatchdogCoord` lock guarantees no send on the reclaimed pool is running). The cross-fs caller re-filters the away-shed list to drives still unmounted at reclaim time (S3 — a drive that reconnected mid-run keeps its connected chain) and stashes its `ReclaimOutcome` for the teardown to record on the single DB connection (M1 — no DB connection on the watchdog thread). |
| `WatchdogCoord` | The single executor↔watchdog coordination cell (UPI 065-b): `{ in_flight: Option<PathBuf>, tripped: HashSet<PathBuf> }`, both fields under **one** `Mutex`. The executor publishes the running send's snapshot root into `in_flight` and refuses a send whose root is in `tripped`, under the same lock the watchdog trips through — so the watchdog's same-vs-cross-filesystem decision and the new-send gate are atomic. The lock admits only two interleavings (executor publishes first → same-fs abort; watchdog trips first → the send is skipped), which makes "no concurrent reclaim of a filesystem a send is reading" a theorem. **Pool identity is the filesystem's full root-set** (`ArmedPool.roots`), and the same-fs test is *membership*, not equality against one representative path — a UUID-pool can span several snapshot roots (C1). Replaces UPI 033's global executor-shutdown new-send gate. |
| `WatchdogAbort` | The ADR-114 event (`EventKind::Watchdog`, `Severity::Warn`) the watchdog records: `{ pool_label, snapshots_reclaimed, send_aborted }` (the `reason` and `freed_reserve` fields were dropped with the cliff + reserve in UPI 067; old rows still deserialize — `EventPayload` has no `deny_unknown_fields`). `send_aborted` (UPI 065-b) discriminates the pool-scoped response: `true` = same-filesystem, the in-flight send was cancelled (UPI 033 behaviour); `false` = cross-filesystem, the send read a different pool and was left running while this pool was relieved concurrently. **`send_aborted` defaults to `true` on the read-old-data path (S2)** — historical rows predate the field and were all same-fs aborts, so the serde default preserves their meaning (the language default `false` would silently relabel them). Event-only surface (no Prometheus/heartbeat field → no homelab ADR-021 amendment). Rendered "guard stopped send on …" (same-fs) / "guard relieved … (left the running send untouched)" (cross-fs) by `voice_events`; a `Critical`-urgency notification accompanies it, with prose aligned to whether a backup was stopped and what was reclaimed. |
...
| `emergency eject` | ADR-113 **Layer 3** (UPI 034): the always-on sentinel's *idle* counterpart to the mid-op watchdog. On its own ~60 s timer, when a source pool has crossed the `catastrophic floor` **with no backup running**, the sentinel sheds the pool's send-enabled, offsite-confirmed local snapshots by reusing `executor::emergency_reclaim_pool` (the same two-tier graduated `abort-reclaim` the watchdog uses post-abort — away pins first, blanket only if the floor still demands it; UPI 058). The sentinel's **first filesystem-mutating action**. Pure floor verdict in `guard::evaluate_idle_eject`; the sequencing decisions (timer gate, eject verdict, backup deferral, re-confirm-under-lock, per-pool order) live in the pure eject protocol `sentinel::eject_transition` (UPI 087), while the runner samples and executes effects (lock, statvfs, reclaim, surfacing). Defers to a running backup via a try-lock on the backup lock path; re-confirms under the lock before acting. **Trusts a confirmed pin as proof of the offsite copy** (a pin exists only because a real send completed) and does **not** re-verify against the often-absent drive — accepted under ADR-113's catastrophic-floor doctrine, trading a negligible stale-pin risk for coverage of the drive-absent case. Event + notification only → zero cross-repo impact. |
...
| `World` | The command-layer prelude (`commands/world.rs`, UPI 082-b): owns the long-lived read-side pieces (`Option<StateDb>` best-effort + read-only `RealBtrfs`) that the `Observation` bundle borrows. Two layers: `view(now) -> WorldView { signals, assessments }` for pure-read surfaces (`status`, bare `urd`, `doctor`), and `fs()`/`observation(&fs)` accessors for the planning paths (`plan`, `backup`). The **sole sanctioned production caller** of `advice::assess_view` (clippy `disallowed-methods` enforced, like the `assess` guard). Extends this cluster's Observation language: `Observation` is the read-only world a pure function observes; `World` is the impure owner that opens it. |
