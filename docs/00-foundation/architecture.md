...
| `commands/seal.rs` | Thin I/O: the earning — `resume_seal` stages unprivileged `visudo -c` → consent → staged install to `urd.staging` → root-side byte-compare against what was shown → root-side `visudo -c` → atomic `mv` → probe/coverage cross-check; hosts the shared probe seam (`probe_grant`, `probe_unsealed`) other surfaces call | Decide grant content (`sudoers.rs` does that); render prompts or mythic voice |
