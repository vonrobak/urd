...
| `sentinel.rs` | Pure state machine for the Sentinel daemon (events, actions, `CircuitState` — the wire type for the `SentinelCircuitBreak` event contract, permanently zero — the machinery behind it was deleted, #385) plus the idle emergency-eject protocol (ADR-113 Layer 3, `eject_transition`: ~60 s timer gate, eject verdict, backup deferral, re-confirm-under-lock sequencing) | Perform I/O (`sentinel_runner.rs` does that) |
