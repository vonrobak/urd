...
<user> ALL=(root) NOPASSWD: /usr/sbin/btrfs qgroup show /
```

For `btrfs filesystem usage` (if used for accurate destination free space):
```
<user> ALL=(root) NOPASSWD: /usr/sbin/btrfs filesystem usage /run/media/<user>/*
<user> ALL=(root) NOPASSWD: /usr/sbin/btrfs filesystem usage /mnt/btrfs-pool
```
