...
mod tests {
    ...
    // generated by serializing the enum, because the point is to catch a
    // future field rename/removal against frozen text, not against
    // whatever the code happens to produce today. When adding a new
    // `EventPayload` variant: add a fixture here (`covered_by_fixture`
    // below refuses to compile until its match gains a matching arm). When
    // adding a field to an existing variant: give it `#[serde(default)]`
    // and add a second, omitted-field fixture proving the historical form
    // still deserializes (see `RetentionPrune`'s pair, and
    // `watchdog_abort_old_format_with_retired_fields_deserializes` above).
    #[test]
    fn event_payload_historical_fixtures_deserialize() {
        let cases: Vec<(&str, &str, EventPayload)> = vec![
            (
                "RetentionPrune (current, with tier)",
                r#"{"type":"RetentionPrune","snapshot":"20260101-0400-home","rule":"graduated_daily","tier":"daily"}"#,
                EventPayload::RetentionPrune {
            ...
            (
                "RetentionPrune (historical, tier field omitted)",
                r#"{"type":"RetentionPrune","snapshot":"20260101-0400-home","rule":"beyond_window"}"#,
                EventPayload::RetentionPrune {
                    snapshot: "20260101-0400-home".into(),
                    rule: PruneRule::BeyondWindow,
                    tier: None,
                },
            ),
            (
                "RetentionProtect",
                r#"{"type":"RetentionProtect","snapshot":"20260101-0400-home","reason":"pin_overrode_thinning"}"#,
                EventPayload::RetentionProtect {
                    snapshot: "20260101-0400-home".into(),
                    reason: ProtectReason::PinOverrodeThinning,
                },
            ...
            (
                "PlannerDefer",
                r#"{"type":"PlannerDefer","reason":"drive not mounted","scope":"drive"}"#,
                EventPayload::PlannerDefer {
            ...
            (
                "PromiseTransition",
                r#"{"type":"PromiseTransition","from":"AT RISK","to":"PROTECTED","trigger":"tick"}"#,
                EventPayload::PromiseTransition {
            ...
            (
                "SentinelCircuitBreak (closed -> open)",
                r#"{"type":"SentinelCircuitBreak","from":"closed","to":"open","reason":"5 consecutive failures","backoff_secs":30}"#,
                EventPayload::SentinelCircuitBreak {
                    from: CircuitState::Closed,
                    to: CircuitState::Open,
                    reason: "5 consecutive failures".into(),
                    backoff_secs: 30,
                },
            ),
            (
                // CircuitState::HalfOpen wire form captured by serializing it
                // directly (`#[serde(rename_all = "lowercase")]` lowercases the
                // whole identifier with no separator: "halfopen", not
                // "half_open") — no existing test pinned this multi-word case.
                "SentinelCircuitBreak (open -> half_open trial)",
                r#"{"type":"SentinelCircuitBreak","from":"open","to":"halfopen","reason":"backoff elapsed, trial allowed","backoff_secs":60}"#,
                EventPayload::SentinelCircuitBreak {
                    from: CircuitState::Open,
                    to: CircuitState::HalfOpen,
                    reason: "backoff elapsed, trial allowed".into(),
                    backoff_secs: 60,
                },
            ),
            (
                "SentinelAnomaly",
                r#"{"type":"SentinelAnomaly","description":"heartbeat gap exceeded threshold"}"#,
                EventPayload::SentinelAnomaly {
                    description: "heartbeat gap exceeded threshold".into(),
                },
            ),
            (
                ...
                "ConfigReloaded",
                r#"{"type":"ConfigReloaded","config_version":"2","source":"/etc/urd/urd.toml"}"#,
                EventPayload::ConfigReloaded {
                    config_version: "2".into(),
                    source: "/etc/urd/urd.toml".into(),
                },
            ),
            (
                "ConfigReloadFailed",
                r#"{"type":"ConfigReloadFailed","reason":"invalid toml at line 12"}"#,
                EventPayload::ConfigReloadFailed {
                    reason: "invalid toml at line 12".into(),
                },
            ),
            (
                "DriveMounted",
                r#"{"type":"DriveMounted","detected_by":"sentinel"}"#,
                EventPayload::DriveMounted {
                    detected_by: DriveEventSource::Sentinel,
                },
            ),
            (
                "DriveUnmounted",
                r#"{"type":"DriveUnmounted","detected_by":"backup"}"#,
                EventPayload::DriveUnmounted {
                    detected_by: DriveEventSource::Backup,
                },
            ),
            (
                "WatchdogAbort (current format)",
                r#"{"type":"WatchdogAbort","pool_label":"/home","snapshots_reclaimed":2,"send_aborted":false}"#,
                EventPayload::WatchdogAbort {
            ...
            (
                "EmergencyEject",
                r#"{"type":"EmergencyEject","pool_label":"/data","free_bytes_before":1000,"floor_bytes":2000,"snapshots_reclaimed":3}"#,
                EventPayload::EmergencyEject {
                    pool_label: "/data".into(),
                    free_bytes_before: 1_000,
                    floor_bytes: 2_000,
            ...
            (
                "OffsiteChainReleased",
                r#"{"type":"OffsiteChainReleased","subvolume":"home","drive":"WD-18TB","parent":"20260101-0400-home"}"#,
                EventPayload::OffsiteChainReleased {
                    subvolume: "home".into(),
                    drive: "WD-18TB".into(),
                    parent: "20260101-0400-home".into(),
                },
            ),
            (
                // Real producer values: `TightnessTier::as_db_str()` is the
                // canonical wire form for `from`/`to` (matching
                // `RetentionPrune.tier`, per this variant's doc comment) —
                // "roomy" / "tight" / "critical", never "normal".
                "StorageTierTransition",
                r#"{"type":"StorageTierTransition","pool_label":"/data","from":"roomy","to":"tight","host_root":false}"#,
        ...
        for (name, json, expected) in cases {
            let actual: EventPayload = serde_json::from_str(json)
                .unwrap_or_else(|e| panic!("{name} fixture failed to deserialize: {e}"));
            assert_eq!(actual, expected, "{name} fixture mismatch");
            // Live exhaustiveness guard (not just a compile-time no-op): this
            // match has no wildcard arm, so adding a new `EventPayload`
            // variant fails the build right here until both this match and
            // the fixture table above account for it.
            covered_by_fixture(&expected);
        }
    }

    /// Called from every case in `event_payload_historical_fixtures_deserialize`
    /// — its wildcard-free match is the compile-time tripwire: a new
    /// `EventPayload` variant fails the build here until this match (and a
    /// fixture in the table above) accounts for it.
    fn covered_by_fixture(payload: &EventPayload) {
        match payload {
            EventPayload::RetentionPrune { .. }
            | EventPayload::RetentionProtect { .. }
            | EventPayload::PlannerSendChoice { .. }
